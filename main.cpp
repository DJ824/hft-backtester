...
int main()
{
    try
    {
        const std::filesystem::path data_file = std::filesystem::current_path() / ".." / "data" / "es0801.csv";

        std::cout << "Parsing " << data_file << "..." << std::endl;
        auto parser = std::make_unique<Parser>(data_file.string());
        parser->parse();

        std::cout << "Creating ingester with " << parser->message_stream_.size() << " messages..." << std::endl;
