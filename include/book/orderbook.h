...
private:
    ...
    inline void add_order(uint64_t id, int32_t price, uint32_t sz, uint64_t ts, bool is_bid);
    inline void modify_order(uint64_t id, int32_t price, uint32_t sz, uint64_t ts, bool is_bid);
    inline void cancel_order(uint64_t id, int32_t price, uint32_t sz, bool is_bid);
...
inline Orderbook::Orderbook(int32_t min_px, int32_t max_px)
    : MIN_(min_px), MAX_(max_px), RANGE_(static_cast<size_t>(MAX_ - MIN_ + 1)),
      bids_(RANGE_, nullptr), asks_(RANGE_, nullptr), order_pool_(1'000'000),
...
inline void Orderbook::process_msg(const book_message& m) {
    ...
    auto nanoseconds = std::chrono::nanoseconds(m.time_);
    ...
    switch (m.action_) {
    case 'A':
        add_order(m.id_, m.price_, m.size_, m.time_, is_bid);
        break;
    case 'M':
        modify_order(m.id_, m.price_, m.size_, m.time_, is_bid);
        break;
    case 'C':
        cancel_order(m.id_, m.price_, m.size_, is_bid);
        break;
    case 'T':
...
inline void Orderbook::add_order(uint64_t id, int32_t price, uint32_t sz, uint64_t ts, bool is_bid) {
    Limit* limit = is_bid ? get_or_insert_limit<false>(price)
                          : get_or_insert_limit<true>(price);

    Order* new_order = order_pool_.get_order();
    new_order->id_ = id;
    new_order->price_ = price;
    new_order->size = sz;
    new_order->side_ = is_bid;
    ...
    if (is_bid) {
        best_bid_idx_ = std::min(best_bid_idx_, static_cast<int32_t>(get_bid_idx(price)));
    } else {
        best_ask_idx_ = std::min(best_ask_idx_, static_cast<int32_t>(get_ask_idx(price)));
    }
}

inline void Orderbook::modify_order(uint64_t id, int32_t price, uint32_t sz, uint64_t ts, bool is_bid) {
    Order* target = order_lookup_.find(id);
    if (!target) {
        add_order(id, price, sz, ts, is_bid);
        return;
    }

    int32_t old = target->price_;
    Limit* target_limit = target->parent_;

    if (old != price) {
        cancel_order(id, old, target->size, is_bid);
        add_order(id, price, sz, ts, is_bid);
        return;
    }

    int32_t diff = static_cast<int32_t>(sz) - (target->size);
    target_limit->volume_ += diff;
    target->size = sz;
    target->unix_time_ = ts;
}

inline void Orderbook::cancel_order(uint64_t id, int32_t /*price*/, uint32_t /*sz*/, bool is_bid) {
    Order* target = order_lookup_.find(id);
    if (!target) {
        return;
    }
    ...
    is_bid ? adjust_bbo<false>() : adjust_bbo<true>();
}
...
inline void Orderbook::calculate_vols(size_t ct) {
    ...
    for (size_t i = 0; i < ct && bid_idx < RANGE_ && ask_idx < RANGE_; i++) {
        if (bids_[bid_idx]) bid_vol_ += bids_[bid_idx]->volume_;
        if (asks_[ask_idx]) ask_vol_ += asks_[ask_idx]->volume_;
        ++bid_idx;
        ++ask_idx;
    }
}
...
inline void Orderbook::print_top_levels(std::size_t depth) const {
    ...
    for (std::size_t printed = 0; printed < depth; ++printed) {
        ...
        if (bid)
            std::cout << std::setw(9) << bid->price_ << '\t' << std::setw(6) << bid->volume_;
        else
        ...
        if (ask)
            std::cout << std::setw(6) << ask->volume_ << '\t' << std::setw(9) << ask->price_;
        else
